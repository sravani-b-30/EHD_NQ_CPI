...
if brand_selection == "EUROPEAN_HOME_DESIGNS":
    ...
    static_file_name = "ehd_scrapped_product_details_final.csv"
else:  # NAPQUEEN
    s3_folder = "NAPQUEEN/"
    price_data_prefix = "napqueen_price_tracker"
    static_file_name = "NAPQUEEN.csv"

# Store the previous selected brand in session state
if 'prev_selected_brand' not in st.session_state:
    st.session_state['prev_selected_brand'] = brand_selection

# Check if the selected brand has changed
if st.session_state['prev_selected_brand'] != brand_selection:
    # Clear session state data when brand selection changes
    st.session_state['result_df'] = None
    st.session_state['show_features_df'] = None
    st.session_state['competitor_files'] = {}
    
    # Optionally, clear other cached data related to the previous brand
    st.session_state['recompute'] = True
    
    # Update the session state with the new selected brand
    st.session_state['prev_selected_brand'] = brand_selection
...
def run_analysis_button(merged_data_df, price_data_df, asin, price_min, price_max, target_price, start_date, end_date, same_brand_option, compulsory_features):
    ...
    if (brand_selection == "NAPQUEEN" and target_brand.upper() == "NAPQUEEN") or \
   (brand_selection == "EUROPEAN_HOME_DESIGNS" and target_brand.upper() == "AMERICAN KENNEL CLUB"):

    # Perform time-series analysis if both start_date and end_date are provided
        ...
        # Perform scatter plot only if dates are not provided
           perform_scatter_plot(asin, target_price, price_min, price_max, compulsory_features, same_brand_option, df_recent, compulsory_keywords)
